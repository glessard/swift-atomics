...
{
  private var val: Int = 0
  public init(_ v: UInt = 0) { val = unsafeBitCast(v, Int.self) }
  public init(integerLiteral value: IntegerLiteralType) { val = unsafeBitCast(UInt(bitPattern: value), Int.self) }
...
extension AtomicUInt
{
  ...
  {
    return unsafeBitCast(ReadWord(&val, order.order), UInt.self)
  }

  @inline(__always)
  public mutating func store(value: UInt, order: StoreMemoryOrder = .relaxed)
  {
    StoreWord(unsafeBitCast(value, Int.self), &val, order.order)
  }

  @inline(__always)
  public mutating func swap(value: UInt, order: MemoryOrder = .relaxed) -> UInt
  {
    return unsafeBitCast(SwapWord(unsafeBitCast(value, Int.self), &val, order.order), UInt.self)
  }

  @inline(__always)
  public mutating func add(delta: UInt, order: MemoryOrder = .relaxed) -> UInt
  {
    return unsafeBitCast(AddWord(unsafeBitCast(delta, Int.self), &val, order.order), UInt.self)
  }

  @inline(__always)
  public mutating func increment(order: MemoryOrder = .relaxed) -> UInt
  {
    return unsafeBitCast(AddWord(1, &val, order.order), UInt.self)
  }

  @inline(__always)
  public mutating func subtract(delta: UInt, order: MemoryOrder = .relaxed) -> UInt
  {
    return unsafeBitCast(SubWord(unsafeBitCast(delta, Int.self), &val, order.order), UInt.self)
  }

  @inline(__always)
  public mutating func decrement(order: MemoryOrder = .relaxed) -> UInt
  {
    return unsafeBitCast(SubWord(1, &val, order.order), UInt.self)
  }

  @inline(__always)
  public mutating func logicalOr(bits: UInt, order: MemoryOrder = .relaxed) -> UInt
  {
    return unsafeBitCast(OrWord(unsafeBitCast(bits, Int.self), &val, order.order), UInt.self)
  }

  @inline(__always)
  public mutating func logicalXor(bits: UInt, order: MemoryOrder = .relaxed) -> UInt
  {
    return unsafeBitCast(XorWord(unsafeBitCast(bits, Int.self), &val, order.order), UInt.self)
  }

  @inline(__always)
  public mutating func logicalAnd(bits: UInt, order: MemoryOrder = .relaxed) -> UInt
  {
    return unsafeBitCast(AndWord(unsafeBitCast(bits, Int.self), &val, order.order), UInt.self)
  }
                                   ...
                                   orderFailure: LoadMemoryOrder = .relaxed) -> Bool
  {
    ...
    switch type {
    case .strong:
      return CASWord(&expect, unsafeBitCast(future, Int.self), &val, orderSuccess.order, orderFailure.order)
    case .weak:
      return CASWeakWord(&expect, unsafeBitCast(future, Int.self), &val, orderSuccess.order, orderFailure.order)
    }
  }
}
...
{
  private var val: Int32 = 0
  public init(_ v: UInt32 = 0) { val = unsafeBitCast(v, Int32.self) }
  public init(integerLiteral value: IntegerLiteralType) { val = unsafeBitCast(UInt32(value), Int32.self) }
...
extension AtomicUInt32
{
  ...
  {
    return unsafeBitCast(Read32(&val, order.order), UInt32.self)
  }

  @inline(__always)
  public mutating func store(value: UInt32, order: StoreMemoryOrder = .relaxed)
  {
    Store32(unsafeBitCast(value, Int32.self), &val, order.order)
  }

  @inline(__always)
  public mutating func swap(value: UInt32, order: MemoryOrder = .relaxed) -> UInt32
  {
    return unsafeBitCast(Swap32(unsafeBitCast(value, Int32.self), &val, order.order), UInt32.self)
  }

  @inline(__always)
  public mutating func add(delta: UInt32, order: MemoryOrder = .relaxed) -> UInt32
  {
    return unsafeBitCast(Add32(unsafeBitCast(delta, Int32.self), &val, order.order), UInt32.self)
  }

  @inline(__always)
  public mutating func increment(order: MemoryOrder = .relaxed) -> UInt32
  {
    return unsafeBitCast(Add32(1, &val, order.order), UInt32.self)
  }

  @inline(__always)
  public mutating func subtract(delta: UInt32, order: MemoryOrder = .relaxed) -> UInt32
  {
    return unsafeBitCast(Sub32(unsafeBitCast(delta, Int32.self), &val, order.order), UInt32.self)
  }

  @inline(__always)
  public mutating func decrement(order: MemoryOrder = .relaxed) -> UInt32
  {
    return unsafeBitCast(Sub32(1, &val, order.order), UInt32.self)
  }

  @inline(__always)
  public mutating func bitwiseOr(bits: UInt32, order: MemoryOrder = .relaxed) -> UInt32
  {
    return unsafeBitCast(Or32(unsafeBitCast(bits, Int32.self), &val, order.order), UInt32.self)
  }

  @inline(__always)
  public mutating func bitwiseXor(bits: UInt32, order: MemoryOrder = .relaxed) -> UInt32
  {
    return unsafeBitCast(Xor32(unsafeBitCast(bits, Int32.self), &val, order.order), UInt32.self)
  }

  @inline(__always)
  public mutating func bitwiseAnd(bits: UInt32, order: MemoryOrder = .relaxed) -> UInt32
  {
    return unsafeBitCast(And32(unsafeBitCast(bits, Int32.self), &val, order.order), UInt32.self)
  }
                                   ...
                                   orderFailure: LoadMemoryOrder = .relaxed) -> Bool
  {
    ...
    switch type {
    case .strong:
      return CAS32(&expect, unsafeBitCast(future, Int32.self), &val, orderSuccess.order, orderFailure.order)
    case .weak:
      return CASWeak32(&expect, unsafeBitCast(future, Int32.self), &val, orderSuccess.order, orderFailure.order)
    }
  }
}
...
{
  private var val: Int64 = 0
  public init(_ v: UInt64 = 0) { val = unsafeBitCast(v, Int64.self) }
  public init(integerLiteral value: IntegerLiteralType) { val = unsafeBitCast(UInt64(value), Int64.self) }
...
extension AtomicUInt64
{
  ...
  {
    return unsafeBitCast(Read64(&val, order.order), UInt64.self)
  }

  @inline(__always)
  public mutating func store(value: UInt64, order: StoreMemoryOrder = .relaxed)
  {
    Store64(unsafeBitCast(value, Int64.self), &val, order.order)
  }

  @inline(__always)
  public mutating func swap(value: UInt64, order: MemoryOrder = .relaxed) -> UInt64
  {
    return unsafeBitCast(Swap64(unsafeBitCast(value, Int64.self), &val, order.order), UInt64.self)
  }

  @inline(__always)
  public mutating func add(delta: UInt64, order: MemoryOrder = .relaxed) -> UInt64
  {
    return unsafeBitCast(Add64(unsafeBitCast(delta, Int64.self), &val, order.order), UInt64.self)
  }

  @inline(__always)
  public mutating func increment(order: MemoryOrder = .relaxed) -> UInt64
  {
    return unsafeBitCast(Add64(1, &val, order.order), UInt64.self)
  }

  @inline(__always)
  public mutating func subtract(delta: UInt64, order: MemoryOrder = .relaxed) -> UInt64
  {
    return unsafeBitCast(Sub64(unsafeBitCast(delta, Int64.self), &val, order.order), UInt64.self)
  }

  @inline(__always)
  public mutating func decrement(order: MemoryOrder = .relaxed) -> UInt64
  {
    return unsafeBitCast(Sub64(1, &val, order.order), UInt64.self)
  }

  @inline(__always)
  public mutating func bitwiseOr(bits: UInt64, order: MemoryOrder = .relaxed) -> UInt64
  {
    return unsafeBitCast(Or64(unsafeBitCast(bits, Int64.self), &val, order.order), UInt64.self)
  }

  @inline(__always)
  public mutating func bitwiseXor(bits: UInt64, order: MemoryOrder = .relaxed) -> UInt64
  {
    return unsafeBitCast(Xor64(unsafeBitCast(bits, Int64.self), &val, order.order), UInt64.self)
  }

  @inline(__always)
  public mutating func bitwiseAnd(bits: UInt64, order: MemoryOrder = .relaxed) -> UInt64
  {
    return unsafeBitCast(And64(unsafeBitCast(bits, Int64.self), &val, order.order), UInt64.self)
  }
                                   ...
                                   orderFailure: LoadMemoryOrder = .relaxed) -> Bool
  {
    ...
    switch type {
    case .strong:
      return CAS64(&expect, unsafeBitCast(future, Int64.self), &val, orderSuccess.order, orderFailure.order)
    case .weak:
      return CASWeak64(&expect, unsafeBitCast(future, Int64.self), &val, orderSuccess.order, orderFailure.order)
    }
  }
}
